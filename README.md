...
- 이메일 형식이 올바르지 않거나 비밀번호가 7자 미만 16자 초과일 경우에도 해당 입력창 하단에 경고 문구가 나타납니다.

- 회원 가입 버튼을 클릭했을 때 모든 항목을 올바르게 작성한 후 필수 약관 동의에 체크하면 회원가입이 성공하고 로그인 화면으로 이동됩니다.
...
- 아이디를 잃어버렸을 경우 가입한 이메일을 입력하면 아이디를 찾을수 있습니다. 아이디는 알림창에 표시되며, 로그인 화면으로 이동합니다.
